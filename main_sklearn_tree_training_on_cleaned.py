# Since the full_train and full_test datasets were very big in size exceeding
# 16 gb in total
...
response = s3_client.get_object(Bucket="www.g43865368.gwu.edu",
                                Key="full_train.csv")
...
# Assign Features and Target Columns to variables 'features' and 'target' to be
# used in training the model
features = list(train.values[:, 4:])
target = list(train.values[:, 3])

# Assign the decision tree classifier to variable 'clf'
clf = tree.DecisionTreeClassifier()

# Train the Decison Tree model according to training dataset Features and
# Targets
...
# Predict Target Probability for the test dataset
...
final_pred.to_csv('forth_submission.csv', index=False, float_format='%.5f')
