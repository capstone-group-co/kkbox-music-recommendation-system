...
# Run a for loop from range 0.01 to 0.99 to get the best threshold or cut off point for prediction
threshold=[]
...
# According to the above for loop the best cutoff point is 0.49, so i will set the final prediction to be 1 if >0.49
my_pred['final_prediction']= (my_pred.predicted_rating>0.49)*1
performance=my_pred['final_prediction']==my_pred['actual_rating']
print("Model have predicted",round((sum(performance)/len(performance))*100,2),"%")
